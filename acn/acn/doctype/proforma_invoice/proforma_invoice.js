frappe.ui.form.on("Proforma Invoice", {
    ...
    frappe.db.get_doc("Customer DC", frm.doc.customer_dc_id).then(dc_doc => {
        ...
        const fetch_and_populate = async (so_doc) => {
            ...
            // --- Build account_head -> rate map from item tax templates ---
            const itax_templates = [...new Set(
                (frm.doc.proforma_invoice_table || [])
                    .map(r => r.item_tax_template)
                    .filter(Boolean)
            )];

            const rate_by_account = {};
            if (itax_templates.length > 0) {
                const tmpl_docs = await Promise.all(
                    itax_templates.map(t => frappe.db.get_doc("Item Tax Template", t))
                );
                tmpl_docs.forEach(tmpl => {
                    (tmpl.taxes || []).forEach(tx => {
                        rate_by_account[tx.tax_type] = tx.tax_rate;
                    });
                });
            }

            // --- Populate taxes table from SO, rate from item tax template ---
            ...
            if (so_doc && so_doc.taxes && so_doc.taxes.length > 0) {
                so_doc.taxes.forEach(tax_row => {
                    ...
                    const rate = rate_by_account[tax_row.account_head] || tax_row.rate || 0;
                    tax_child.rate = rate;

                    const tax_amount = (taxable_value * rate) / 100;
        ...
        if (sales_order_no) {
            frappe.db.get_doc("Sales Order", sales_order_no).then(async (so_doc) => {
                await fetch_and_populate(so_doc);
            }).catch(async (err) => {
                console.error("SO fetch error:", err);
                frappe.msgprint(__("Could not fetch Sales Order details. Populating from Customer DC only."));
                await fetch_and_populate(null);
            });
        } else {
